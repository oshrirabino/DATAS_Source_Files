...
private:
    std::unique_ptr<datas::LogAVLTree<int>> tree;
    ...
    void printMenu() {
        if (interactive_mode) {
            ...
            *program_out << "  print           - Display the tree (inorder)\n";
            *program_out << "  structure       - Display tree structure\n";
            ...
            *program_out << "========================\n";
            *program_out << "Current tree size: " << tree_size << "\n";
            program_out->flush();
        }
    }
    ...
    void insertValue(int value) {
        ...
        bool already_exists = tree->exist_in_tree(value);
        if (already_exists) {
            *program_out << "INSERT_DUPLICATE value=" << value << " size=" << tree_size << std::endl;
            return;
        }
    ...
    void removeValue(int value) {
        ...
        bool exists = tree->exist_in_tree(value);
        if (!exists) {
        ...
        try {
            ...
            if (removed) {
                ...
                *program_out << "REMOVE_FAILED value=" << value << " size=" << tree_size << std::endl;
            }
    ...
    void findValue(int value) {
        ...
        try {
            bool found = tree->exist_in_tree(value);
    ...
    void printTree() {
        ...
        *program_out << "TREE_INORDER_START" << std::endl;
        tree->inorder();
        *program_out << "TREE_INORDER_END" << std::endl;
    }
    ...
    void printTreeStructure() {
        ...
        *program_out << "TREE_STRUCTURE_START" << std::endl;
        tree->printTreeStructure();
        *program_out << "TREE_STRUCTURE_END" << std::endl;
    }
    ...
    void initTree() {
        tree = std::make_unique<datas::LogAVLTree<int>>(log_stream);
        tree_size = 0;
        log_stream.str("");
        log_stream.clear();
        
        *program_out << "INIT_SUCCESS type=AVL size=" << tree_size << std::endl;
    }

public:
    AVLTreeInterface(bool interactive = true) 
        : tree_size(0), interactive_mode(interactive),
    ...
    void run() {
        ...
        if (interactive_mode) {
            *program_out << "AVL Tree Interface Started" << std::endl;
            printMenu();
        } else {
            *program_out << "READY type=AVL" << std::endl;
        }
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (arg == "--batch") {
            ...
            std::cout << "\nCommands:\n";
            std::cout << "  init             - Initialize new tree\n";
            std::cout << "  insert <value>   - Insert a value\n";
            std::cout << "  remove <value>   - Remove a value\n";
            std::cout << "  find <value>     - Search for a value\n";
            std::cout << "  print            - Display the tree (inorder traversal)\n";
            std::cout << "  structure        - Display tree structure with hierarchy\n";
            ...
            std::cout << "  " << argv[0] << " --batch --tree-log-out avl.log\n";
            std::cout << "  \n";
            ...
            std::cout << "  " << argv[0] << " --batch --program-out program.log --tree-log-out avl_tree.log\n";
            return 0;
        }
    }
    
    try {
        AVLTreeInterface interface(interactive);
